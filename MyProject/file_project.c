...
int main(void) {
    ...
    // 비밀번호 : 나영코딩 skdudzheld
    printf("\n\n === 비밀번호 확인 중... ===\n\n");
    if (strcmp(password, "skdudzheld") == 0) { // 비밀번호 일치
        printf(" === 비밀번호 확인 완료 === \n\n");
        char * fileName = "/Users/nayomi/Repos/secretdiary.txt";
        FILE * file = fopen(fileName, "a+b"); // a+b : 파일이 없으면 생성, 파일이 있으면 append 를 한다. (뒤에서부터 내용을 추가한다)
        if (file == NULL) {
            printf("파일 열기 실패\n");
            return 1;
        }
